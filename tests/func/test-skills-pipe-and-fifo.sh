#!/usr/bin/env bash
# Functional tests for pipe-and-fifo skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/pipe-and-fifo/SKILL.md"
...
echo "=== Tests: pipe-and-fifo skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: pipe-and-fifo$' "$FILE"
