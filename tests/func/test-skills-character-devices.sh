#!/usr/bin/env bash
# Functional tests for character-devices skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/character-devices/SKILL.md"
...
echo "=== Tests: character-devices skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: character-devices$' "$FILE"
