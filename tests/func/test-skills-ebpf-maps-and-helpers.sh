#!/usr/bin/env bash
# Functional tests for ebpf-maps-and-helpers skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/ebpf-maps-and-helpers/SKILL.md"
...
echo "=== Tests: ebpf-maps-and-helpers skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: ebpf-maps-and-helpers$' "$FILE"
