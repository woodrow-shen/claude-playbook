#!/usr/bin/env bash
# Functional tests for vfs-layer skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/vfs-layer/SKILL.md"
...
echo "=== Tests: vfs-layer skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: vfs-layer$' "$FILE"
