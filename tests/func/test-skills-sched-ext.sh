#!/usr/bin/env bash
# Functional tests for sched-ext skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/sched-ext/SKILL.md"
...
echo "=== Tests: sched-ext skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: sched-ext$' "$FILE"
