#!/usr/bin/env bash
# Functional tests for cpu-topology-and-load-balancing skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/cpu-topology-and-load-balancing/SKILL.md"
...
echo "=== Tests: cpu-topology-and-load-balancing skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: cpu-topology-and-load-balancing$' "$FILE"
