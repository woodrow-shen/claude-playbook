#!/usr/bin/env bash
# Functional tests for io-uring skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/io-uring/SKILL.md"
...
echo "=== Tests: io-uring skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: io-uring$' "$FILE"
