#!/usr/bin/env bash
# Functional tests for tcp-congestion-control skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/tcp-congestion-control/SKILL.md"
...
echo "=== Tests: tcp-congestion-control skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: tcp-congestion-control$' "$FILE"
