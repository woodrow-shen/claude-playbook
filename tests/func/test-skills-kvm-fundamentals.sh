#!/usr/bin/env bash
# Functional tests for kvm-fundamentals skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/kvm-fundamentals/SKILL.md"
...
echo "=== Tests: kvm-fundamentals skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: kvm-fundamentals$' "$FILE"
