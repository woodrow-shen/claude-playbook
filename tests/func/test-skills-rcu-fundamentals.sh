#!/usr/bin/env bash
# Functional tests for rcu-fundamentals skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/rcu-fundamentals/SKILL.md"
...
echo "=== Tests: rcu-fundamentals skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: rcu-fundamentals$' "$FILE"
