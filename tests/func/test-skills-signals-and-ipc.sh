#!/usr/bin/env bash
# Functional tests for signals-and-ipc skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/signals-and-ipc/SKILL.md"
...
echo "=== Tests: signals-and-ipc skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: signals-and-ipc$' "$FILE"
