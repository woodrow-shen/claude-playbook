#!/usr/bin/env bash
# Functional tests for boot-and-init skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/boot-and-init/SKILL.md"
...
echo "=== Tests: boot-and-init skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: boot-and-init$' "$FILE"
