#!/usr/bin/env bash
# Functional tests for socket-layer skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/socket-layer/SKILL.md"
...
echo "=== Tests: socket-layer skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: socket-layer$' "$FILE"
