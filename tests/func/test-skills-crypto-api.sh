#!/usr/bin/env bash
# Functional tests for crypto-api skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/crypto-api/SKILL.md"
...
echo "=== Tests: crypto-api skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: crypto-api$' "$FILE"
