#!/usr/bin/env bash
# Functional tests for ext4-internals skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/ext4-internals/SKILL.md"
...
echo "=== Tests: ext4-internals skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: ext4-internals$' "$FILE"
