#!/usr/bin/env bash
# Functional tests for page-cache-and-readahead skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/page-cache-and-readahead/SKILL.md"
...
echo "=== Tests: page-cache-and-readahead skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: page-cache-and-readahead$' "$FILE"
