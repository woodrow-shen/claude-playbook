#!/usr/bin/env bash
# Functional tests for v7-scheduler-changes skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/v7-scheduler-changes/SKILL.md"
...
echo "=== Tests: v7-scheduler-changes skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: v7-scheduler-changes$' "$FILE"
