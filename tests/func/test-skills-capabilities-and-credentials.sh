#!/usr/bin/env bash
# Functional tests for capabilities-and-credentials skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/capabilities-and-credentials/SKILL.md"
...
echo "=== Tests: capabilities-and-credentials skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: capabilities-and-credentials$' "$FILE"
