#!/usr/bin/env bash
# Functional tests for kernel-modules skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/kernel-modules/SKILL.md"
...
echo "=== Tests: kernel-modules skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: kernel-modules$' "$FILE"
