#!/usr/bin/env bash
# Functional tests for v7-bpf-changes skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/v7-bpf-changes/SKILL.md"
...
echo "=== Tests: v7-bpf-changes skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: v7-bpf-changes$' "$FILE"
