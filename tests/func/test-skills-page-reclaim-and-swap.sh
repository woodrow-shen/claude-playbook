#!/usr/bin/env bash
# Functional tests for page-reclaim-and-swap skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/page-reclaim-and-swap/SKILL.md"
...
echo "=== Tests: page-reclaim-and-swap skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: page-reclaim-and-swap$' "$FILE"
