#!/usr/bin/env bash
# Functional tests for virtual-memory-areas skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/virtual-memory-areas/SKILL.md"
...
echo "=== Tests: virtual-memory-areas skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: virtual-memory-areas$' "$FILE"
