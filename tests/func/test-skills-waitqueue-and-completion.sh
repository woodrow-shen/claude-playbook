#!/usr/bin/env bash
# Functional tests for waitqueue-and-completion skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/waitqueue-and-completion/SKILL.md"
...
echo "=== Tests: waitqueue-and-completion skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: waitqueue-and-completion$' "$FILE"
