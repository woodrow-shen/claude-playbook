#!/usr/bin/env bash
# Functional tests for v7-memory-changes skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/v7-memory-changes/SKILL.md"
...
echo "=== Tests: v7-memory-changes skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: v7-memory-changes$' "$FILE"
