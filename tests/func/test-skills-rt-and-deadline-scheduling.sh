#!/usr/bin/env bash
# Functional tests for rt-and-deadline-scheduling skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/rt-and-deadline-scheduling/SKILL.md"
...
echo "=== Tests: rt-and-deadline-scheduling skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: rt-and-deadline-scheduling$' "$FILE"
