#!/usr/bin/env bash
# Functional tests for perf-events skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/perf-events/SKILL.md"
...
echo "=== Tests: perf-events skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: perf-events$' "$FILE"
