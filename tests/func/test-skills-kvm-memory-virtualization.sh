#!/usr/bin/env bash
# Functional tests for kvm-memory-virtualization skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/kvm-memory-virtualization/SKILL.md"
...
echo "=== Tests: kvm-memory-virtualization skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: kvm-memory-virtualization$' "$FILE"
