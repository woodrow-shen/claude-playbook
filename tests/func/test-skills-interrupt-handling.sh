#!/usr/bin/env bash
# Functional tests for interrupt-handling skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/interrupt-handling/SKILL.md"
...
echo "=== Tests: interrupt-handling skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: interrupt-handling$' "$FILE"
