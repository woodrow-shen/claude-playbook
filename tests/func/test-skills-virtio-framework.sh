#!/usr/bin/env bash
# Functional tests for virtio-framework skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/virtio-framework/SKILL.md"
...
echo "=== Tests: virtio-framework skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: virtio-framework$' "$FILE"
