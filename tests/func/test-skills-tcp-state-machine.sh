#!/usr/bin/env bash
# Functional tests for tcp-state-machine skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/tcp-state-machine/SKILL.md"
...
echo "=== Tests: tcp-state-machine skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: tcp-state-machine$' "$FILE"
