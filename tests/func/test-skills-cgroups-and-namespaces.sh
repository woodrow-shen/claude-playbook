#!/usr/bin/env bash
# Functional tests for cgroups-and-namespaces skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/cgroups-and-namespaces/SKILL.md"
...
echo "=== Tests: cgroups-and-namespaces skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: cgroups-and-namespaces$' "$FILE"
