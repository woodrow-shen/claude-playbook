#!/usr/bin/env bash
# Functional tests for memcg-and-oom skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/memcg-and-oom/SKILL.md"
...
echo "=== Tests: memcg-and-oom skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: memcg-and-oom$' "$FILE"
