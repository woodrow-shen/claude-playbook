#!/usr/bin/env bash
# Functional tests for pci-and-dma skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/pci-and-dma/SKILL.md"
...
echo "=== Tests: pci-and-dma skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: pci-and-dma$' "$FILE"
