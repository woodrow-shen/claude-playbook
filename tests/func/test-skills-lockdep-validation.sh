#!/usr/bin/env bash
# Functional tests for lockdep-validation skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/lockdep-validation/SKILL.md"
...
echo "=== Tests: lockdep-validation skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: lockdep-validation$' "$FILE"
