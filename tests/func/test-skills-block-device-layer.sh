#!/usr/bin/env bash
# Functional tests for block-device-layer skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/block-device-layer/SKILL.md"
...
echo "=== Tests: block-device-layer skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: block-device-layer$' "$FILE"
