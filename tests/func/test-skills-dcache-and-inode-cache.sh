#!/usr/bin/env bash
# Functional tests for dcache-and-inode-cache skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/dcache-and-inode-cache/SKILL.md"
...
echo "=== Tests: dcache-and-inode-cache skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: dcache-and-inode-cache$' "$FILE"
