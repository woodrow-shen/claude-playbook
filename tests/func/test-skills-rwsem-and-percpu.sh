#!/usr/bin/env bash
# Functional tests for rwsem-and-percpu skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/rwsem-and-percpu/SKILL.md"
...
echo "=== Tests: rwsem-and-percpu skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: rwsem-and-percpu$' "$FILE"
