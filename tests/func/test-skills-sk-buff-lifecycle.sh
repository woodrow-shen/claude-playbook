#!/usr/bin/env bash
# Functional tests for sk-buff-lifecycle skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/sk-buff-lifecycle/SKILL.md"
...
echo "=== Tests: sk-buff-lifecycle skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: sk-buff-lifecycle$' "$FILE"
