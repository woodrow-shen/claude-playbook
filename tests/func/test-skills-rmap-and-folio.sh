#!/usr/bin/env bash
# Functional tests for rmap-and-folio skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/rmap-and-folio/SKILL.md"
...
echo "=== Tests: rmap-and-folio skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: rmap-and-folio$' "$FILE"
