#!/usr/bin/env bash
# Functional tests for slab-allocator skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/slab-allocator/SKILL.md"
...
echo "=== Tests: slab-allocator skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: slab-allocator$' "$FILE"
