#!/usr/bin/env bash
# Functional tests for process-lifecycle skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/process-lifecycle/SKILL.md"
...
echo "=== Tests: process-lifecycle skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: process-lifecycle$' "$FILE"
