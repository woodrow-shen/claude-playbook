#!/usr/bin/env bash
# Functional tests for spinlocks-and-mutexes skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/spinlocks-and-mutexes/SKILL.md"
...
echo "=== Tests: spinlocks-and-mutexes skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: spinlocks-and-mutexes$' "$FILE"
