#!/usr/bin/env bash
# Functional tests for kbuild-and-kconfig skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/kbuild-and-kconfig/SKILL.md"
...
echo "=== Tests: kbuild-and-kconfig skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: kbuild-and-kconfig$' "$FILE"
