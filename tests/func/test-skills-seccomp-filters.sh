#!/usr/bin/env bash
# Functional tests for seccomp-filters skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/seccomp-filters/SKILL.md"
...
echo "=== Tests: seccomp-filters skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: seccomp-filters$' "$FILE"
