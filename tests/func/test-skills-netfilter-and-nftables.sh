#!/usr/bin/env bash
# Functional tests for netfilter-and-nftables skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/netfilter-and-nftables/SKILL.md"
...
echo "=== Tests: netfilter-and-nftables skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: netfilter-and-nftables$' "$FILE"
