#!/usr/bin/env bash
# Functional tests for timers-and-hrtimers skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/timers-and-hrtimers/SKILL.md"
...
echo "=== Tests: timers-and-hrtimers skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: timers-and-hrtimers$' "$FILE"
