#!/usr/bin/env bash
# Functional tests for cgroups-v2 skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/cgroups-v2/SKILL.md"
...
echo "=== Tests: cgroups-v2 skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: cgroups-v2$' "$FILE"
