#!/usr/bin/env bash
# Functional tests for system-calls skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/system-calls/SKILL.md"
...
echo "=== Tests: system-calls skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: system-calls$' "$FILE"
