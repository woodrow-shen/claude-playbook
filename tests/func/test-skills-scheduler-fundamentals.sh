#!/usr/bin/env bash
# Functional tests for scheduler-fundamentals skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/scheduler-fundamentals/SKILL.md"
...
echo "=== Tests: scheduler-fundamentals skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: scheduler-fundamentals$' "$FILE"
