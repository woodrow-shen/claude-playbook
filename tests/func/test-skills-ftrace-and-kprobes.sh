#!/usr/bin/env bash
# Functional tests for ftrace-and-kprobes skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/ftrace-and-kprobes/SKILL.md"
...
echo "=== Tests: ftrace-and-kprobes skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: ftrace-and-kprobes$' "$FILE"
