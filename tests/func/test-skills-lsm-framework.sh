#!/usr/bin/env bash
# Functional tests for lsm-framework skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/lsm-framework/SKILL.md"
...
echo "=== Tests: lsm-framework skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: lsm-framework$' "$FILE"
