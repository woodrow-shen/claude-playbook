#!/usr/bin/env bash
# Functional tests for futex-and-locking skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/futex-and-locking/SKILL.md"
...
echo "=== Tests: futex-and-locking skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: futex-and-locking$' "$FILE"
