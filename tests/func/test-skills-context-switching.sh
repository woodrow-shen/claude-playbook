#!/usr/bin/env bash
# Functional tests for context-switching skill
# Validates the SKILL.md structure and frontmatter.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FILE="$SCRIPT_DIR/../../configs/linux/.claude/skills/context-switching/SKILL.md"
...
echo "=== Tests: context-switching skill ==="

assert "File exists" test -f "$FILE"
assert "Has name in frontmatter" grep -q '^name: context-switching$' "$FILE"
